/**
 * BẢN ĐỒ MẢNG NGHIỆP VỤ → NHÓM TUYẾN. Tệp DUY NHẤT giữ ánh xạ này.
 *
 * Vì sao chỉ một tệp: có test đối chiếu bản đồ này với `danh-muc.json` và bắt buộc MỌI nhóm
 * trong danh mục phải thuộc ĐÚNG MỘT mảng. Thêm nhóm mới ở `clazzi-api`, quét lại, mà quên xếp
 * vào mảng ⇒ test đỏ ngay. Không có hàng rào đó thì tuyến mới lặng lẽ không công cụ nào gọi tới
 * được, và sẽ chẳng ai phát hiện ra cho tới lúc có người hỏi "sao không làm được việc này".
 *
 * Thứ tự nhóm trong mỗi mảng có ý nghĩa: nhóm ĐẦU TIÊN là mặc định gợi ý khi mô hình không ghi
 * `doiTuong`, nên hãy để nhóm "chính" của mảng lên trước.
 */

export interface Mang {
  /** Tên công cụ MCP đầy đủ. */
  ten: string;
  /** Mô tả công cụ — mô hình đọc cái này để chọn công cụ, nên phải nói đúng việc nghiệp vụ. */
  moTa: string;
  /** Các tiền tố nhóm thuộc mảng, theo đúng `tienTo` trong `danh-muc.json`. */
  nhom: string[];
}
...
export const MANG: Mang[] = [
  ...
  },
  {
    ...
    nhom: ['/session-exercises', '/session-exercise-attachments', '/session-exercise-report', '/session-exercise-report-attachments', '/exercise-comments'],
  },
  {
    ten: 'clazzi_hoc_phi',
  ...
  {
    ten: 'clazzi_bao_cao',
    moTa: 'Bảng điều khiển và số liệu tổng hợp toàn trung tâm.',
    nhom: ['/dashboard'],
  },
  {
...
/** Tra nhanh nhóm → mảng. Dựng một lần lúc nạp module. */
const THEO_NHOM = new Map<string, Mang>();
for (const m of MANG) {
  for (const n of m.nhom) {
    const da = THEO_NHOM.get(n);
    // Một nhóm nằm ở hai mảng thì `clazzi_mo_ta` và thông điệp lỗi sẽ chỉ sai đường; chặn ngay
    // lúc nạp còn hơn để nó thành lỗi mơ hồ lúc chạy.
    if (da) throw new Error(`Nhóm ${n} bị xếp vào cả ${da.ten} lẫn ${m.ten}`);
    THEO_NHOM.set(n, m);
  }
}

export function mangCuaNhom(tienTo: string): Mang | undefined {
  return THEO_NHOM.get(tienTo);
}

export function timMang(ten: string): Mang | undefined {
  return MANG.find((m) => m.ten === ten);
}
