...
import { diaChiGoiCua, khoaBatBuoc, type Phien } from './phien';
...
export async function goiApi({ phien, method, duongDan, thamSo, than }: ThamSoGoi): Promise<KetQuaGoi> {
  const khoa = khoaBatBuoc(phien);
  const goc = diaChiGoiCua(phien);
  ...
  try {
    ...
    throw new LoiNguoiDung(`Không gọi được ${method} ${u.pathname} tại bản ${phien.ban.ten} (${phien.ban.diaChi}): ${ly}`);
  } finally {
    clearTimeout(hen);
  }
  ...
  if (!phanHoi.ok) throw new LoiNguoiDung(dichLoi(phien.ban, phanHoi.status, duLieu, method, u.pathname));
