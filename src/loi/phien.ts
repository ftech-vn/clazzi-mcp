...
import { nhanDienBan } from './ban';
import { LoiCauHinh } from './loi';
...
export function diaChiGoiCua(phien: Phien): string {
...
export function phienTuMoiTruong(): Phien {
  const c = cauHinh();
  return { ban: nhanDienBan(process.env[c.bienUrl]), khoa: process.env[c.bienKhoa]?.trim() || null };
}
 ...
 */
export function dauKhoa(phien: Phien): string | null {
  if (!phien.khoa) return null;
  const tt = cauHinh().tienToKhoa;
 ...
 */
export function khoaBatBuoc(phien: Phien): string {
  if (!phien.khoa) throw new LoiCauHinh(thieuKhoa(phien));
  return phien.khoa;
}

function thieuKhoa(phien: Phien): string {
  const { ban } = phien;
  const c = cauHinh();
