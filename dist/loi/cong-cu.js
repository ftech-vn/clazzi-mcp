...
exports.dinhNghiaCongCu = dinhNghiaCongCu;
exports.chayCongCu = chayCongCu;
const danh_muc_1 = require("./danh-muc");
const http_1 = require("./http");
const ket_qua_1 = require("./ket-qua");
const loi_1 = require("./loi");
const mang_1 = require("./mang");
 ...
 */
const tenGoi = () => `${(0, cau_hinh_1.cauHinh)().tienTo}_goi`;
const tenMoTa = () => `${(0, cau_hinh_1.cauHinh)().tienTo}_mo_ta`;
const tenTrangThai = () => `${(0, cau_hinh_1.cauHinh)().tienTo}_trang_thai`;
const VIEC_ENUM = ['liet_ke', 'xem', 'tao', 'sua', 'xoa'];
/** Mô tả một nhóm trong phần mô tả enum: tên tiếng Việt + đúng những việc nó làm được. */
function dongDoiTuong(n) {
    const co = (0, danh_muc_1.vieclamDuoc)(n);
...
function nhomBatBuoc(tienTo) {
    const n = (0, danh_muc_1.timNhom)(tienTo);
    if (!n) {
        const gan = (0, danh_muc_1.napDanhMuc)()
            .nhom.map(x => x.tienTo)
            .filter(x => x.includes((0, danh_muc_1.chuanHoaNhom)(tienTo).replace(/^\//, '').slice(0, 4)))
            .slice(0, 5);
        throw new loi_1.LoiNguoiDung(`Không có đối tượng "${tienTo}" trong danh mục.${gan.length ? ` Gần đúng: ${gan.join(', ')}` : ''}`);
    }
    return n;
}
/** Lược đồ tham số dùng chung cho 24 công cụ nghiệp vụ. */
function luocDoNghiepVu(nhomCuaMang) {
    return {
        type: 'object',
        properties: {
            doiTuong: {
...
function dinhNghiaCongCu(loc = {}) {
    const ra = [];
    const bat = loc.moduleDangBat;
    for (const m of (0, mang_1.moiMang)()) {
        /**
        ...
        inputSchema: {
            ...
            properties: {
                doiTuong: {
                    ...
                    enum: (0, danh_muc_1.napDanhMuc)().nhom.map(n => n.tienTo),
                },
            },
            required: ['doiTuong'],
            additionalProperties: false,
...
async function chayNghiepVu(phien, tenMang, ts) {
    const mang = (0, mang_1.timMang)(tenMang);
    /* istanbul ignore next */ if (!mang)
        throw new loi_1.LoiNguoiDung(`Không có công cụ "${tenMang}".`);
    if (!ts.doiTuong)
        throw new loi_1.LoiNguoiDung(`Thiếu "doiTuong". Mảng ${mang.ten} có: ${mang.nhom.join(', ')}.`);
    ...
    if ((0, mang_1.mangCuaNhom)(nhom.tienTo)?.ten !== mang.ten) {
        throw new loi_1.LoiNguoiDung(`Đối tượng ${nhom.tienTo} không thuộc ${mang.ten} mà thuộc ${(0, mang_1.mangCuaNhom)(nhom.tienTo)?.ten}. Gọi đúng công cụ đó.`);
    }
    const viec = ts.viec ? danh_muc_1.VIEC_TU_TEN[ts.viec] : undefined;
    if (!viec)
        throw new loi_1.LoiNguoiDung(`Thiếu hoặc sai "viec". Nhận một trong: ${VIEC_ENUM.join(', ')}.`);
    // Ném lỗi nói rõ nếu nhóm không có việc này — hàng rào chính, đừng bọc lại.
    const tuyen = (0, danh_muc_1.tuyenCuaViec)(nhom, viec);
    const [method, mau] = tuyen.split(' ');
    if ((viec === 'xem' || viec === 'sua' || viec === 'xoa') && !ts.id) {
        throw new loi_1.LoiNguoiDung(`Việc "${danh_muc_1.TEN_VIEC[viec]}" cần "id" (tuyến ${tuyen}).`);
    }
    if ((viec === 'tao' || viec === 'sua') && (!ts.duLieu || Object.keys(ts.duLieu).length === 0)) {
        throw new loi_1.LoiNguoiDung(`Việc "${danh_muc_1.TEN_VIEC[viec]}" cần "duLieu" không rỗng. Xem ${tenMoTa()} ${nhom.tienTo} để biết các trường.`);
    }
    ...
    const kq = await (0, http_1.goiApi)({ phien, method, duongDan, thamSo, than });
    return (0, ket_qua_1.ketQua)(phien, { doiTuong: nhom.tienTo, viec: danh_muc_1.TEN_VIEC[viec], tuyen: `${method} ${duongDan}`, ketQua: kq.duLieu });
}
async function chayTrangThai(phien) {
    const dm = (0, danh_muc_1.napDanhMuc)();
    const chung = {
        ban: { ten: phien.ban.ten, diaChi: phien.ban.diaChi, laHeThat: phien.ban.laHeThat },
        danhMuc: { quetLuc: dm.quetLuc, soTuyen: dm.soTuyen, soNhom: dm.soNhom, soCongCu: (0, mang_1.moiMang)().length + 3 },
        khoa: phien.khoa
            ? { daDat: true, nhanDang: (0, phien_1.dauKhoa)(phien), ghiChu: '12 ký tự prefix, đủ để đối chiếu khoá nào, không đủ để dùng' }
            : { daDat: false },
    };
    if (!phien.khoa) {
        return (0, ket_qua_1.ketQua)(phien, { ...chung, ketNoi: 'Chưa có khoá API nên chưa kiểm được. Xem hướng dẫn khi gọi bất kỳ công cụ nghiệp vụ nào.' });
    }
    ...
    for (const duongDan of ['/staffs/profile', '/users/profile']) {
        try {
            const kq = await (0, http_1.goiApi)({ phien, method: 'GET', duongDan });
            return (0, ket_qua_1.ketQua)(phien, { ...chung, ketNoi: 'OK', taiKhoanTheo: duongDan, taiKhoan: kq.duLieu });
        }
        catch (e) {
            if (duongDan === '/users/profile') {
                 ...
                 */
                return (0, ket_qua_1.ketQua)(phien, {
                    ...chung,
    ...
    /* istanbul ignore next */ return (0, ket_qua_1.ketQua)(phien, chung);
}
function chayMoTa(phien, doiTuong) {
    const nhom = nhomBatBuoc(doiTuong);
    const mang = (0, mang_1.mangCuaNhom)(nhom.tienTo);
    return (0, ket_qua_1.ketQua)(phien, {
        doiTuong: nhom.tienTo,
        ten: nhom.ten,
        module: nhom.module,
        congCu: mang?.ten,
        crud: Object.fromEntries(Object.keys(danh_muc_1.TEN_VIEC).map(v => [danh_muc_1.TEN_VIEC[v], nhom.crud[v] ?? 'KHÔNG CÓ — đừng gọi việc này'])),
...
function doGiong(a, b) {
    const ta = new Set(a.split(/[/-]/).filter(Boolean));
    const tb = b.split(/[/-]/).filter(Boolean);
    return tb.filter(x => ta.has(x)).length;
}
async function chayGoi(phien, ts) {
    const method = (ts.method ?? '').toUpperCase();
    const duongDan = (ts.duongDan ?? '').split('?')[0] ?? '';
    if (!method || !duongDan)
        throw new loi_1.LoiNguoiDung(`${tenGoi()} cần cả "method" lẫn "duongDan".`);
    const tatCa = (0, danh_muc_1.moiTuyen)();
    ...
    const kq = await (0, http_1.goiApi)({ phien, method, duongDan, thamSo: ts.thamSo, than: ts.than });
    return (0, ket_qua_1.ketQua)(phien, { tuyen: `${method} ${duongDan}`, khopMau: `${khop.method} ${khop.path}`, nhom: khop.nhom, ketQua: kq.duLieu });
}
/**
 ...
 */
async function chayCongCu(phien, ten, thamSo) {
    try {
        if (ten === tenTrangThai())
            return { text: await chayTrangThai(phien), loi: false };
        if (ten === tenMoTa())
            return { text: chayMoTa(phien, String(thamSo.doiTuong ?? '')), loi: false };
        if (ten === tenGoi())
            return { text: await chayGoi(phien, thamSo), loi: false };
        if ((0, mang_1.timMang)(ten))
            return { text: await chayNghiepVu(phien, ten, thamSo), loi: false };
        throw new loi_1.LoiNguoiDung(`Không có công cụ "${ten}".`);
    }
    catch (e) {
        if (e instanceof loi_1.LoiNguoiDung)
            return { text: (0, ket_qua_1.ketQuaLoi)(phien, e.message), loi: true };
        // Lỗi lập trình: vẫn gắn tên bản, nhưng để nguyên thông điệp để còn gỡ được.
        return { text: (0, ket_qua_1.ketQuaLoi)(phien, `Lỗi không lường trước: ${e instanceof Error ? e.message : String(e)}`), loi: true };
    }
}
//# sourceMappingURL=cong-cu.js.map