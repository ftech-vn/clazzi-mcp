...
exports.nhomCuaDuongDan = nhomCuaDuongDan;
exports.timTepController = timTepController;
exports.docBanKeModule = docBanKeModule;
exports.quetTep = quetTep;
exports.xepCrud = xepCrud;
exports.quetKho = quetKho;
 ...
 */
const PHUONG_THUC = ['Get', 'Post', 'Put', 'Delete', 'Patch'];
 ...
 */
function laMotThamSo(path, tienTo) {
    if (!path.startsWith(`${tienTo}/`))
        return false;
    return path.slice(tienTo.length + 1).replace(/\/$/, '') === ':id';
}
/** Đúng tiền tố, không thêm đoạn nào (chấp nhận cả `/courses` và `/courses/`). */
function laGoc(path, tienTo) {
    return path === tienTo || path === `${tienTo}/`;
}
/** Đọc mọi tệp `.controller.ts` trong một thư mục (không đệ quy — cây nguồn phẳng một tầng). */
function liet(thuMuc) {
    if (!(0, node_fs_1.existsSync)(thuMuc))
        return [];
    return (0, node_fs_1.readdirSync)(thuMuc)
        .filter(f => f.endsWith('.controller.ts'))
        .sort()
        .map(f => (0, node_path_1.join)(thuMuc, f));
}
function timTepController(goc) {
    const tep = liet((0, node_path_1.join)(goc, 'src', 'controllers'));
    const thuMucModule = (0, node_path_1.join)(goc, 'src', 'modules');
    if ((0, node_fs_1.existsSync)(thuMucModule)) {
        for (const m of (0, node_fs_1.readdirSync)(thuMucModule).sort()) {
            tep.push(...liet((0, node_path_1.join)(thuMucModule, m, 'controllers')));
        }
    }
    return tep;
}
 ...
 */
function docBanKeModule(goc) {
    const ra = [];
    const thuMucModule = (0, node_path_1.join)(goc, 'src', 'modules');
    if (!(0, node_fs_1.existsSync)(thuMucModule))
        return ra;
    for (const m of (0, node_fs_1.readdirSync)(thuMucModule).sort()) {
        const tep = (0, node_path_1.join)(thuMucModule, m, 'ban-ke.ts');
        if (!(0, node_fs_1.existsSync)(tep))
            continue;
        const noiDung = (0, node_fs_1.readFileSync)(tep, 'utf8');
        const re = /tienTo:\s*['"`](.*?)['"`]\s*,\s*module:\s*['"`](.*?)['"`]/g;
        let k;
        while ((k = re.exec(noiDung)) !== null)
            ra.push({ tienTo: k[1], module: k[2] });
    }
    return ra;
}
/**
 ...
 */
function moduleCuaNhom(tienTo, banKe) {
...
function quetTep(duongDanTep) {
    const dong = (0, node_fs_1.readFileSync)(duongDanTep, 'utf8').split('\n');
    ...
    for (let i = 0; i < dong.length; i++) {
        ...
        if (!mTuyen)
            continue;
        const method = mTuyen[1].toUpperCase();
        ...
        for (let j = i + 1; j < dong.length; j++) {
            ...
            for (let k = j; k < dong.length; k++) {
                chuKy += `${dong[k]}\n`;
                if (/^\s{0,4}\)/.test(dong[k]) || /\)\s*(?::[^{]*)?\{\s*$/.test(dong[k]))
                    break;
            }
            let q;
            RE_QUERY_PARAM.lastIndex = 0;
            while ((q = RE_QUERY_PARAM.exec(chuKy)) !== null)
                thamSoTruyVan.push(q[1]);
...
function xepCrud(tienTo, tuyen) {
    const crud = { lietKe: null, xem: null, tao: null, sua: null, xoa: null };
    ...
    gan('lietKe', tuyen.find(t => t.method === 'GET' && laGoc(t.path, tienTo)));
    gan('xem', tuyen.find(t => t.method === 'GET' && laMotThamSo(t.path, tienTo)));
    gan('tao', tuyen.find(t => t.method === 'POST' && laGoc(t.path, tienTo)));
    gan('sua', tuyen.find(t => t.method === 'PUT' && laMotThamSo(t.path, tienTo)));
    gan('xoa', tuyen.find(t => t.method === 'DELETE' && laMotThamSo(t.path, tienTo)));
 ...
 */
function quetKho(gocClazziApi, tenNhom = {}) {
    const banKe = docBanKeModule(gocClazziApi);
    const moiTuyen = [];
    for (const tep of timTepController(gocClazziApi))
        moiTuyen.push(...quetTep(tep));
    ...
    const nhom = [...theoNhom.keys()].sort().map(tienTo => {
...
//# sourceMappingURL=quet-nguon.js.map